...
const ModalGioHang = () => {
  const { danhSachSanPhamGioHang, xoaSanPham,tangGiamSL } = useContext(GioHangContextHook);
  const renderSanPhamTrongGioHang = () => {
    let result = null;
    if (danhSachSanPhamGioHang < 0) return result;
    return (result = danhSachSanPhamGioHang.map((sp, index) => {
      return (
        <tr key={index}>
          <td>{sp.id}</td>
          <td>{sp.name}</td>
